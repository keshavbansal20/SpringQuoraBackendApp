...
public class AnswerService implements IAnswerService {
    ...
    public Mono<AnswerResponseDTO> updateAnswer(String id, AnswerRequestDTO answerRequestDTO) {
        return answerRepository.findById(id)
                ...
                .map(this::mapToResponseDTO)
                .switchIfEmpty(Mono.error(new RuntimeException("Answer not found with it: "+id)));
    }
    ...
    public Mono<Void> deleteAnswer(String id) {
        return answerRepository.findById(id)
                .switchIfEmpty(Mono.error(new RuntimeException("Answer not found with id: "+id)))
                .flatMap(existingAnswer -> answerRepository.deleteById(id));
    }
    ...
    public Flux<AnswerResponseDTO> getAllAnswersByQuestionId(String questoinId) {
            return answerRepository.findByQuestionId(questoinId).map(this::mapToResponseDTO);
    }
    ...
    public Flux<AnswerResponseDTO> getAnswersByQuestionIdOrderByCreatedAtDesc(String questionId) {
        return answerRepository.findByQuestionIdOrderByCreatedAtDesc(questionId).map(this::mapToResponseDTO);
    }

    @Override
    public Flux<AnswerResponseDTO> getAnswersByQuestionIdOrderByCreatedAtAsc(String questionId) {
        return answerRepository.findByQuestionIdOrderByCreatedAtAsc(questionId).map(this::mapToResponseDTO);
    }
